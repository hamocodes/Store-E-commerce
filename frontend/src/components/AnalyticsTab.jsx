			...
			>
				<ResponsiveContainer width='100%' height={400}>
					<LineChart data={dailySalesData}>
						...
						<Tooltip />
						<Legend />
						<Line yAxisId='left' type='monotone' dataKey='sales' stroke='#F3F4F6' activeDot={{ r: 8 }} name='Sales' />
						<Line yAxisId='right' type='monotone' dataKey='revenue' stroke='#9CA3AF' activeDot={{ r: 8 }} name='Revenue' />
					</LineChart>
				</ResponsiveContainer>
			</motion.div>
		</div>
...
const AnalyticsCard = ({ title, value, icon: Icon }) => (
	<motion.div
		className='bg-gray-900 rounded-lg p-6 shadow-lg overflow-hidden relative'
		initial={{ opacity: 0, y: 20 }}
		animate={{ opacity: 1, y: 0 }}
